...
func GetAllChatSessionsForJob(channel chan<- []models.ChatSession) {
	...
	defer db.Close()
	rows, err := db.Query("SELECT session_id,title,allow_web_search,image_generation FROM chat_sessions")
	if err != nil {
...
func SearchChatSessions(userId string, searchVector []float32, channel chan<- []models.ChatSession) {
	...
	rows, err := db.Query("SELECT session_id,title,allow_web_search,img_generation FROM chat_sessions WHERE user_id = ? AND vector_distance_cos(title_vector, vector32(?)) < 0.5 ORDER BY vector_distance_cos(title_vector, vector32(?))", userId, vectorStr, vectorStr)
	if err != nil {
		fmt.Printf("Failed to execute query: %v\n", err.Error())
		channel <- data
		return
...
func UpdateChatSessionWebSearchFlag(userId string, sessionId int, allowWebSearch bool, channel chan<- int) {
	db, err := createDb()
	if err != nil {
		channel <- 0
		return
	...
	defer db.Close()
	result, err := db.Exec("UPDATE chat_sessions SET allow_web_search = ? WHERE session_id = ? AND  user_id = ?", allowWebSearch, sessionId, userId)
	if err != nil {
		fmt.Printf("Failed to execute query: %v\n", err.Error())
		channel <- 0
		return
...
func UpdateChatSessionImgGenerationFlag(userId string, sessionId int, imgGeneration bool, channel chan<- int) {
	db, err := createDb()
	if err != nil {
		channel <- 0
		return
	...
	defer db.Close()
	result, err := db.Exec("UPDATE chat_sessions SET img_generation = ? WHERE session_id = ? AND  user_id = ?", imgGeneration, sessionId, userId)
	if err != nil {
		fmt.Printf("Failed to execute query: %v\n", err.Error())
		channel <- 0
		return
	...
	rowsAffected, errUpdate := result.RowsAffected()
	if errUpdate != nil {
		fmt.Printf("Error updating img_generation : %v\n", errUpdate.Error())
		channel <- 0
		return
	}
	channel <- int(rowsAffected)
...
func GetChatConversations(userId string, sessionId int, channel chan<- []models.ChatConversation) {
	...
	defer db.Close()
	rows, err := db.Query("SELECT DISTINCT conversation_id,chat_conversations.session_id,message,sender,file_data,file_name FROM chat_conversations INNER JOIN chat_sessions ON chat_sessions.session_id=chat_conversations.session_id WHERE chat_sessions.session_id = ? AND user_id=? ORDER BY timestamp", sessionId, userId)
	if err != nil {
		fmt.Printf("Failed to execute query: %v\n", err.Error())
		channel <- data
		return
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.Id, &item.SessionId, &item.Message, &item.Sender, &item.FileData, &item.FileName); err != nil {
			fmt.Printf("Error scanning row:%v\n", err.Error())
		} else {
			data = append(data, item)
		}
