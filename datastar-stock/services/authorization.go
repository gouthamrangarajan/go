...
func SignInEmailPassword(email, password string, channel chan<- models.SignInResponse) {
	...
	if err != nil {
		fmt.Println("Error reading sign-in response body:", err)
		signInResponse.ErrorMessage = err.Error()
		channel <- signInResponse
		return
	}
...
func SignInWithRefreshToken(refreshToken string, channel chan<- models.SignInRefreshTokenResponse) {
	signInResponse := models.SignInRefreshTokenResponse{}
	signInUrl := os.Getenv("GOOGLE_IDENTITY_SIGNIN_REFRESH_TOKEN_URL")
	jsonBody, err := json.Marshal(models.SignInRefreshTokenRequest{
		GrantType:    "refresh_token",
		RefreshToken: refreshToken,
	})
	if err != nil {
		fmt.Println("Error marshalling refresh token  sign-in request:", err)
		signInResponse.ErrorMessage = err.Error()
		channel <- signInResponse
		return
	}
	resp, err := http.Post(signInUrl, "application/json", bytes.NewBuffer(jsonBody))
	if err != nil {
		fmt.Println("Error making refresh token sign-in request:", err)
		signInResponse.ErrorMessage = err.Error()
		channel <- signInResponse
		return
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		fmt.Println("Refresh token sign-in failed with status code:", resp.StatusCode)
		signInResponse.ErrorMessage = fmt.Sprintf("Sign-in failed with status code: %d", resp.StatusCode)
		channel <- signInResponse
		return
	}
	bodyBytes, err := io.ReadAll(resp.Body)
	if err != nil {
		fmt.Println("Error reading refresh token sign-in response body:", err)
		signInResponse.ErrorMessage = err.Error()
		channel <- signInResponse
		return
	}
	err = json.Unmarshal(bodyBytes, &signInResponse)
	if err != nil {
		fmt.Println("Error unmarshalling refresh token sign-in response:", err)
		signInResponse.ErrorMessage = err.Error()
		channel <- signInResponse
		return
	}
	errorGettingEmailModel := models.SignInRefreshTokenResponse{}
	firebaseConfig := models.FirebaseConfig{
		Type:                    os.Getenv("FIREBASE_TYPE"),
		ProjectID:               os.Getenv("FIREBASE_PROJECT_ID"),
		PrivateKeyID:            os.Getenv("FIREBASE_PRIVATE_KEY_ID"),
	...
	if firebaseConfigErr != nil {
		fmt.Println("Error marshalling FirebaseConfig:", firebaseConfigErr)
		channel <- errorGettingEmailModel
		return
	}
	app, appErr := firebase.NewApp(context.Background(), nil, option.WithCredentialsJSON(
		firebaseConfigJson,
	...
	tokenParsed, err := auth.VerifyIDToken(ctx, signInResponse.IDToken)
	if err != nil {
		fmt.Println("Error verifying id token", err)
		channel <- errorGettingEmailModel
		return
	}
	signInResponse.Email = tokenParsed.Firebase.Identities["email"].([]any)[0].(string)
	channel <- signInResponse
}
