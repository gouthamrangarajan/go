...
func getNotesHandler(responseWriter http.ResponseWriter, request *http.Request) {
	...
	sse.PatchElementTempl(components.EditorList(data), datastar.WithModeAppend(), datastar.WithSelectorID("section"), datastar.WithUseViewTransitions(true))
}
...
func addNoteHandler(responseWriter http.ResponseWriter, request *http.Request) {
	...
	channel := make(chan models.NoteData)
	defer close(channel)
	go services.InsertNote(accessToken, channel)
	note := <-channel
	if note.Id != "" {
		sse := datastar.NewSSE(responseWriter, request)
