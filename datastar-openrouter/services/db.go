...
func SearchChatSessions(userId string, searchVector []float32, channel chan<- []models.ChatSession) {
	...
	rows, err := db.Query("SELECT session_id,title,allow_web_search FROM chat_sessions WHERE user_id = ? AND vector_distance_cos(title_vector, vector32(?)) < 0.9 ORDER BY vector_distance_cos(title_vector, vector32(?))", userId, vectorStr, vectorStr)
	if err != nil {
